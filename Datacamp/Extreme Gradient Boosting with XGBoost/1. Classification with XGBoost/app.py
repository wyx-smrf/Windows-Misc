...
st.sidebar.title('Modify XGBoost Parameters')

def model_parameters(model):
    params = dict()
    if model == 'XGBoost':
        objective = st.sidebar.selectbox('Select objective', ['binary:logistic', 'reg:logistic'])
        params['objective'] = objective

        n_estimators = st.sidebar.slider('Pick a number', 0, 50, 10)
        params['n_estimators'] = n_estimators

        seed = st.sidebar.number_input('Pick a number', 0, 10, 5)
        params['seed'] = seed
...
def get_classifier(model, parameters, X_train, X_test, y_train, y_test):
    if model == 'XGBoost':
        clf = xgb.XGBClassifier(params=parameters)   
